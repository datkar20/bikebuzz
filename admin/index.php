...
$pdo = db();
$stats = [
    'Sản phẩm' => ['value' => (int) $pdo->query('SELECT COUNT(*) FROM products')->fetchColumn(), 'icon' => 'bi-box-seam'],
    'Danh mục' => ['value' => (int) $pdo->query('SELECT COUNT(*) FROM categories')->fetchColumn(), 'icon' => 'bi-tags'],
    'Người dùng' => ['value' => (int) $pdo->query('SELECT COUNT(*) FROM users')->fetchColumn(), 'icon' => 'bi-people'],
    'Đánh giá' => ['value' => (int) $pdo->query('SELECT COUNT(*) FROM reviews')->fetchColumn(), 'icon' => 'bi-star'],
];
...
<div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
    <div>
        ...
        <div class="text-muted">Quản lý sản phẩm, danh mục, người dùng và thông báo.</div>
    </div>
...
<div class="bb-card p-3">
    <div class="d-flex justify-content-between align-items-center mb-2">
        <h2 class="h5 fw-bold mb-0">Sản phẩm mới</h2>
        <a class="btn btn-outline-dark btn-sm" href="<?= url('admin/products.php') ?>">Quản lý</a>
    </div>
    <div class="table-responsive">
        <table class="table align-middle mb-0">
            <thead><tr><th>Xe</th><th>Danh mục</th><th>Giá</th><th>Kho</th></tr></thead>
            <tbody>
